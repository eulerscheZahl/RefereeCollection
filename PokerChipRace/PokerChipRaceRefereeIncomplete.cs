...
// for collisions see also: http://files.magusgeek.com/csb/csb_en.html
		...
		private void Simulate()
		{
			...
			while (time < 1)
			{
				...
				for (int c1Index = 0; c1Index < Chips.Count; c1Index++)
				{
					...
					{
						if (chip.Father == Chips[c2Index].Father) continue;
	...
	class Chip
	{
		...
		public Collision CollideEdge()
		{
			...
			if (this.VY > 0) collideEdge = Math.Min(collideEdge, (Board.HEIGHT - this.Y - this.Radius) / this.VY);
			else if (this.VY < 0) collideEdge = Math.Min(collideEdge, -(this.Y - this.Radius) / this.VY);
